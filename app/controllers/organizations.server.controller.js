...
exports.list = function (req, res) {
    Organization.find().sort('-created').populate('user', 'displayName')
        .populate('owner', 'firstName lastName')
        .populate('projects').populate('members', 'firstName lastName').exec(function (err, organizations) {
            if (err) {
                return res.status(400).send({
                    message: errorHandler.getErrorMessage(err)
                });
...
exports.organizationByID = function (req, res, next, id) {
    Organization.findById(id).populate('user', 'displayName').populate('owner', 'firstName lastName')
        .populate('projects').populate('members', 'firstName lastName').exec(function (err, organization) {
